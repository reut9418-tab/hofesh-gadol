...
function detectStructure(rows, learned = {}) {
  ...
  if (best.rowIdx >= 0) {
    ...
    // עמודת חלק-ההרחבה בכל מספר ימים ("עבור 5/6/7 ימים")
    if (best.mapping.extPortion === undefined) {
      const c = headerRowN.findIndex((h) => /^עבור\s*\d+\s*ימים/.test(h));
      if (c >= 0 && learned[headerRowN[c]] !== 'none') best.mapping.extPortion = c;
    }
...
function runChecks(recs, { grossCap = GROSS_CAP.schools_gardens, hoursCap = HOURS_CAP, vatFactor = 1, framework = null } = {}) {
  // בבתי הספר מי שמעל 90 שעות הוא סגן (עד 93) או רכז (מעל 93) —
  // תקרת השעות לא חלה עליהם ואין מה להציף; בדיוק 90 = מורה רגילה
