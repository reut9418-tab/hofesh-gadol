...
async function stage1Data(db, report, client, authority) {
  ...
  const mkUnit = (name, symbol, baskets, salaryNet, split, children, salaryByPayer) => {
    ...
    return {
      ...
      targets: {
        // יעד הכרטסת נגזר מהדוח ביצוע (הסכום שדווח, אחרי התקרות) בניכוי
        // מע"מ — לא מדוח העלות; העלות בספרים מוצגת בנפרד להשוואה
        salary: net(salaryActual),
        booksNet: salaryNet, // העלות בספרים (דוח העלות) — לתצוגה בלבד
        salaryByPayer: Object.fromEntries(Object.entries(salaryByPayer || {}).map(([p, v]) =>
          [p, salaryNet > 0 ? Math.round((v / salaryNet) * net(salaryActual) * 100) / 100 : v])), // פרופורציונלית ליעד
...
function renderStage1Html(d) {
  ...
  const unitOptions = d.units.map((u) => {
    ...
    const booksNote = d.hasVat
      ? ` <span class="soft">(עלות בספרים: ₪${fmt(u.targets.booksNet)}; דווח בדוח הביצוע כולל מע"מ ותקרות: ₪${fmt(u.salaryActual)}; יעד הכרטסת = הדיווח בניכוי מע"מ: ₪${fmt(u.targets.salary)})</span>`
      : '';
